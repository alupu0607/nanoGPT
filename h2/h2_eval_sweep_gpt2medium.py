...
def download_artifact(L, m, task):
    if L == 0:
        return "baseline"
    artifact_name = f"{ENTITY}/{PROJECT}/prefix-L{L}-m{m}-cacheon-{task}-{MODEL_TAG}:latest"
    try:
...
def get_training_metrics(L, m, task):
    run_name = f"prefix-L{L}-m{m}-cacheon-{task}-{MODEL_TAG}"
    try:
        runs = api.runs(
            f"{ENTITY}/{PROJECT}",
...
eval_run = wandb.init(
    project  = PROJECT,
    name     = f"h2-eval-{MODEL_TAG}-{TASK}-cacheon-{timestamp}",
    job_type = "eval",
)
wandb.log({
...
with open(f"/kaggle/working/h2_{MODEL_TAG}_eval_cacheon_summary.json", "w") as f:
    json.dump(results, f, indent=2)
print(f"\nSaved to /kaggle/working/h2_{MODEL_TAG}_eval_cacheon_summary.json")
